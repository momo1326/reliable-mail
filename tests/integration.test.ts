...
async function testIdempotency(emailId: number) {
  ...
  const data1: EmailResponse = await response1.json();
  ...
  const data2: EmailResponse = await response2.json();
  const secondId = data2.id;

  assert.strictEqual(
    firstId,
    secondId,
    `Idempotent requests should return same ID. Got ${firstId}, then ${secondId}`
  );
